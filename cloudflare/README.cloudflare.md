...
## 推荐部署架构（Worker + D1 + KV + R2 + 可选 Docker 混合）

生产推荐把 Cloudflare 原生版作为统一入口，按资源职责拆分：

```text
用户 / OpenAI SDK
   │
   ▼
Cloudflare Worker（Hono 路由、OpenAI 兼容 API、用户/管理 API、限流、JWT/API Key）
   ├── D1：用户、API Keys、账号池、积分流水、调用记录、生成任务 URL
   ├── KV：账号健康状态、短期限流计数、临时会话/小元数据
   ├── R2：生成大图、大视频、长生命周期媒体文件
   └── 可选 Docker 后端：复杂登录、浏览器保活、uTLS/代理场景、灰度兜底
```

建议实践：

1. **纯 Worker 路径**：`/v1/*`、`/api/*`、`/admin/api/*` 全部由 Worker 处理，账号池在 D1，健康状态在 KV，生成媒体落 R2。
2. **混合路径**：GPT/Grok 遇到 Cloudflare 真人挑战、复杂 Cookie 刷新或 SOCKS/TCP 代理需求时，由 Docker 版先完成登录/保活，再通过批量导入 API 同步 Cookie 到 Worker。
3. **媒体路径**：图片/视频只把 R2 URL 写入 D1 `generations.result_url/r2_url`，Worker 返回 URL；不要把大文件写入 KV 或 D1。
4. **灰度切换**：先部署 `cf-api.example.com`，验证账号池、计费和 R2 后，再把主域名路由切到 `[env.production]` 的 Workers Custom Domain。

## `/api/tokens/import` 批量账号导入

除了管理后台路径 `/admin/api/v1/accounts/import`，Cloudflare 版也提供 grok2api 风格短路径：

```bash
curl -X POST https://api.example.com/api/tokens/import \
  -H 'authorization: Bearer ADMIN_JWT' \
  -H 'content-type: application/json' \
  -d '{
    "provider":"grok",
    "tokens":"sso=xxx; sso-rw=xxx\nsso=yyy; sso-rw=yyy",
    "weight":1
  }'
```

GPT OAuth / Cookie 结构化导入：
...
> 安全建议：该接口要求管理员 JWT；生产请限制 `CORS_ALLOW_ORIGINS`，并只在可信网络/后台中使用。

## 生产就绪状态

当前 Cloudflare 原生版已标记为 **Ready for Production**：

- Worker Assets 自动承载用户前台与管理后台，SPA 路由回退已配置。
- D1/KV/R2 分层存储职责明确：D1 存业务和 URL，KV 存短缓存/健康状态，R2 存大图/视频。
- GPT/Grok upstream 具备 Cookie bootstrap、OAuth 刷新、401/403 session retry、429/5xx 重试和账号熔断。
- `wrangler.toml` 已提供 development 与 `[env.production]` 双环境配置，包含 placement、limits、observability、D1、KV、R2 和 Cron。
- 仍建议复杂真人验证、uTLS、SOCKS/TCP CONNECT 代理场景保留 Docker 混合兜底。
