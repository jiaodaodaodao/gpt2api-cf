...
authRoutes.post('/auth/register', async (c) => {
  ...
  if (await getUserByEmail(c.env, email)) return jsonErr('email already exists', 409, 'exists');
...
authRoutes.post('/auth/login', async (c) => {
  ...
  if (!user || user.status !== 'active') return jsonErr('invalid credentials', 401, 'unauthorized');
  ...
  if (!stored || stored.password_hash !== hash) return jsonErr('invalid credentials', 401, 'unauthorized');
