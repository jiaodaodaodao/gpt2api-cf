...
adminRoutes.post('/admin/api/v1/accounts', async (c) => {
  ...
  const recId = id('acc');
  await c.env.DB.prepare('insert into accounts(id,provider,label,cookie_encrypted,access_token_encrypted,proxy_url,upstream_base_url,weight,status,fail_count,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?)')
    .bind(recId, body.provider, body.label || body.provider, await encryptText(body.cookie, c.env.ENCRYPTION_KEY), body.access_token ? await encryptText(body.access_token, c.env.ENCRYPTION_KEY) : null, body.proxy_url || null, body.upstream_base_url || null, Number(body.weight || 1), 'active', 0, nowIso(), nowIso()).run();
