...
import { charge, logApiCall, logRequest } from '../services/db';
import { cacheGeneration, forwardOpenAIRequest, providerForModel } from '../services/upstream';
...
async function forwardCompat(c: any, kind: 'chat' | 'image' | 'video', requiredScope: 'chat' | 'image' | 'video') {
  ...
  try {
    ...
    if (!body.stream && contentType.includes('application/json')) {
      ...
      if (data && (kind === 'image' || kind === 'video')) await cacheGeneration(c.env, kind, data);
