...
function retryAfterMs(res: Response) {
  ...
  if (Number.isFinite(seconds)) return Math.min(seconds * 1000, 3000);
  const ts = Date.parse(raw);
  return Number.isFinite(ts) ? Math.min(Math.max(ts - Date.now(), 0), 3000) : 0;
}
...
async function attemptOpenAIRequest(env: Env, request: Request, provider: Provider, body: ArrayBuffer | undefined, account: AccountRecord) {
...
export async function forwardOpenAIRequest(env: Env, request: Request, provider: Provider) {
  ...
  const maxAttempts = Math.max(1, Math.min(getNum(env.UPSTREAM_RETRY_MAX, 2) + 1, 4));
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    if (!account) throw new Error(`no healthy ${provider} account available${lastError ? `: ${lastError}` : ''}`);
    try {
      const res = await attemptOpenAIRequest(env, request, provider, body, account);
      if (!isRetryableStatus(res.status) || attempt === maxAttempts) {
        await markAccountResult(env, account.id, res.ok);
        return { response: res, accountId: account.id, provider, latencyMs: Date.now() - started, attempts: attempt };
      }
      lastError = `upstream status ${res.status}`;
      await markAccountResult(env, account.id, false, lastError);
      await sleep(retryAfterMs(res) || Math.min(250 * 2 ** (attempt - 1), 1500));
    } catch (e) {
  ...
  throw new Error(lastError || `no healthy ${provider} account available`);
}
